...
function mouseMoved(e) {
  ...
  //
  pointer.degrees = Math.floor((pointer.angle *180)/Math.PI) //find the degrees
  if (pointer.degrees < 0){
    pointer.degrees = 360+pointer.degrees; //if - just add to 360 to get real degrees
  }
  //
...
function handleKeys(e) {
  ...
  if (e.code === 'KeyP') {
    if (window.player.classList.contains('hidden')){
...
function drawPointerPos() {
  ...
  context.arc(pointer.x, pointer.y, pointer.radius / step * 50 , 0, Math.PI * 2, true);
  context.stroke();
...
function drawUserPos() {
  ...
  context.beginPath();
  context.lineWidth = 1;
  context.fillStyle = colours[colourNum];
  context.arc(halfWidth, halfHeight, step/2, 0, Math.PI * 2, true);
  context.fill();

}

function colourCounter(){
  if (colourNum<3){colourNum++;}
  else{colourNum=0;}
...
function init() {
  ...
  //added by me <3
  window.addEventListener("click", colourCounter);
